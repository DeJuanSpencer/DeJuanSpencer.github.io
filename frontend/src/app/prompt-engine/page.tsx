...
export default function PromptEngine() {
  ...
  useEffect(() => {
    const checkIdle = () => {
      if (Date.now() - lastActivity.current > 45000 && step > 0 && step < 9) setShowAssist(true);
    };
...
{"projectName":"...","domain":"...","description":"...","goals":"...","identity":{"title":"...","description":"...","traits":[]},"knowledge":[],"negatives":[{"behavior":"...","instruction":"...","reason":"..."}],"modes":[{"name":"...","trigger":"...","description":"...","characteristics":[]}],"priorities":[{"rule":"...","overrides":"...","exception":"..."}],"failures":[{"pattern":"...","prevention":"...","severity":"medium"}],"templates":[],"examples":[]}`,
  ...
  const copyToClipboard = () => { navigator.clipboard.writeText(compiledOutput); setCopied(true); setTimeout(() => setCopied(false), 2000); };
  const projectBlurb = `${projectDesc}${goals ? " Goal: " + goals.trim().replace(/\.?\s*$/, ".") : ""}`.trim();
  const copyBlurb = () => { navigator.clipboard.writeText(projectBlurb); setCopiedBlurb(true); setTimeout(() => setCopiedBlurb(false), 2000); };
  ...
  const renderPriority = () => (
    <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
        {priorities.map((p, i) => (
          <Card key={i} style={{ display: "flex", alignItems: "center", gap: "12px" }}>
            <div style={{ display: "flex", flexDirection: "column", gap: "2px" }}>
              <button onClick={() => movePriority(i, -1)} style={{ background: "none", border: "none", cursor: "pointer", padding: "2px" }}><ArrowUp size={14} color="#e0e0e0" /></button>
              <button onClick={() => movePriority(i, 1)} style={{ background: "none", border: "none", cursor: "pointer", padding: "2px" }}><ArrowDown size={14} color="#e0e0e0" /></button>
            </div>
  ...
  const renderExamples = () => (
    <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
        {examples.map((e, i) => (
          <Card key={i} highlight={approvedExamples.has(i)}>
  ...
  const renderPreviewPanel = () => (
    <div style={{ width: showPreview ? "300px" : "0px", minWidth: showPreview ? "300px" : "0px", borderLeft: showPreview ? "1px solid rgba(255,255,255,0.06)" : "none", background: "rgba(0,0,0,0.15)", overflow: "hidden", transition: "all 0.3s", display: "flex", flexDirection: "column" }}>
      {showPreview && (
    ...
    <div style={{ minHeight: "100vh", background: "#111113", color: "#e0e0e0", fontFamily: "'DM Sans', sans-serif", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "16px 24px", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", justifyContent: "space-between", alignItems: "center", background: "rgba(0,0,0,0.2)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "14px" }}>
          <div style={{ width: "32px", height: "32px", borderRadius: "8px", background: "linear-gradient(135deg, #d4a24e, #b8862e)", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <Zap size={18} color="#1a1a1a" />
          </div>
          <div>
            <div style={{ fontSize: "16px", fontWeight: 700, letterSpacing: "-0.3px" }}>Prompt Engine</div>
            <div style={{ fontSize: "11px", color: "rgba(255,255,255,0.4)", fontFamily: "'JetBrains Mono', monospace" }}>Project Instructions Builder · {VERSION}</div>
          </div>
        </div>
        <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
          <Badge active={appMode === "create"} onClick={() => { setAppMode("create"); setParsedPreview(null); }}>Create</Badge>
          <Badge active={appMode === "edit"} onClick={() => setAppMode("edit")}>Edit Existing</Badge>
          <div style={{ width: "1px", height: "24px", background: "rgba(255,255,255,0.1)", margin: "0 4px" }} />
          <button onClick={() => setShowPreview(!showPreview)} style={{ background: "none", border: "none", cursor: "pointer", padding: "6px", display: "flex", alignItems: "center", gap: "6px" }}>
            {showPreview ? <PanelRightClose size={18} color="rgba(255,255,255,0.5)" /> : <PanelRightOpen size={18} color="rgba(255,255,255,0.5)" />}
            <span style={{ fontSize: "11px", color: "rgba(255,255,255,0.35)", fontFamily: "'JetBrains Mono', monospace" }}>Preview</span>
          </button>
        </div>
      </div>
